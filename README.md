...
* ⚡ **완전 자동화 (Zero-Keypress):** 모드 로드 즉시 세이브 파일을 선제 패치하고, 백그라운드 가디언 루프(`LoopAsync 500ms`)가 `BP_SkinManager_C` 및 `BP_AchievementManager_C` 스폰을 감지하여 사용자의 키 조작 없이 전자동으로 해금합니다.
